...
function moduleState(m) {
  ...
  if (m.layout !== 'study') return {status:'warn',label:m.layout === 'standard' ? 'Layout mismatch' : 'ABI unavailable',filter:'review'};
...
function demoSnapshot() {
  ...
  return {meta:{model:'Pixel 8 Pro (sample)',android:'15',abi:'arm64-v8a',kernel:'6.1.99-android14',version:'v0.1.0',selinux:'Enforcing',collected:new Date().toISOString(),logStatus:'available'},checks:[...checks,...RUNTIME.map(c=>({...c}))],modules:[
    ...
    {id:'sample_disabled',name:'Disabled example',version:'v0.3.0',author:'Sample module',description:'Illustrates a disable marker. The current daemon does not filter these markers.',abis:'arm64-v8a',layout:'study',flags:'disabled',eligible:'yes'}
...
function showModule(index) {
  ...
  showDialog(`<h2>${escapeHtml(m.name)}</h2>${badge(s.status,s.label)}<p>${escapeHtml(m.description||'No description supplied by this module.')}</p><dl class="device-list">${[['Module ID',m.id],['Version',m.version],['Author',m.author],['Architectures',m.abis],['Disk layout',m.layout],['Manager marker',m.flags],['Daemon path candidate',m.eligible==='yes'?'Yes (disk pattern only)':'No']].map(([k,v])=>`<div><dt>${k}</dt><dd>${escapeHtml(v||'Not supplied')}</dd></div>`).join('')}</dl><div class="notice">${icon('info')}<span>${m.flags!=='enabled'?'A manager marker is present, but the current study daemon does not filter disabled or removing modules. Do not assume it is inactive. ':''}${m.layout==='standard'?'The current daemon does not enumerate this standard layout. Renaming the binary is not enough to make the upstream ABI compatible. ':''}No per-process load telemetry is available. This inventory cannot confirm successful loading or callback execution.</span></div>`);
}
